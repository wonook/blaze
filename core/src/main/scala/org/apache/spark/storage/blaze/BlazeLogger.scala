...
private[spark] object BlazeLogger extends Logging {
  ...
  def cachingDecision(costFun: String, blockId: BlockId, executor: String,
    ...
    if (costFun.contains("GD")) {
      logInfo(s"DECIDED_TO_CACHE\t$blockId\tExecutor $executor\t" +
  ...
  def cachingFailure(blockId: BlockId, executorId: String,
    ...
    logInfo(s"CACHING_FAIL_MEM\tExecutor $executorId\t$blockId\t" +
  ...
  def materialized(blockId: BlockId, stageId: Int, executorId: String, size: Long,
                   onDisk: Boolean): Unit = {
    if (onDisk) {
      logInfo(s"PERSIST_DONE\tStage $stageId\tExecutor $executorId\t$blockId\tSerSize $size")
    } else {
      logInfo(s"CACHING_DONE\tStage $stageId\tExecutor$executorId\t$blockId\tDeserSize $size")
    }
  }

  def promotionDone(blockId: BlockId, stageId: Int, executorId: String): Unit = {
    logInfo(s"PROMOTED\t$blockId\tExecutor $executorId\tStage $stageId")
  }


  def discardDecision(blockId: BlockId, executor: String,
                      prio: Double, size: Long, msg: String): Unit = {
    logInfo(s"DISCARD_MEM\tExecutor $executor\t$blockId\t$prio\t$size\t$msg")
  }
  ...
  def evictionFailure(blockId: BlockId, executorId: String): Unit = {
    logInfo(s"EVICTION_FAIL_MEM\tExecutor $executorId\t$blockId")
  }
  ...
  def removeMemBlock(stageId: Int, blockId: BlockId, executorId: String): Unit = {
    logInfo(s"REMOVE_MEM\tStage $stageId\tExecutor $executorId\t$blockId")
  }

  def removeDiskBlock(stageId: Int, blockId: BlockId, executorId: String): Unit = {
    logInfo(s"REMOVE_DISK\tStage $stageId\tExecutor $executorId\t$blockId")
  }

  /* TODO: fault tolerance? */

  def clearAllMemBlocks(executor: String, aggregateSize: Long): Unit = {
    logInfo(s"CLEAR_ALL_MEM\tExecutor $executor\tAggregateDeserSize $aggregateSize")
  }
  ...
  def unrollingTime(blockId: String, time: Long): Unit = {
    logInfo(s"UNROLL_TIME\t$blockId\t$time")
  }

  def compTime(key: String, time: Long): Unit = {
    logInfo(s"COMP_TIME\t$key\t$time")
  }
  ...
  def cacheHit(blockId: BlockId, executorId: String, fromRemote: Boolean,
    ...
    if (fromRemote) {
      logInfo(s"REMOTE_HIT\t$blockId\tExecutor $executorId\t$readTime")
    } else {
      if (onDisk) {
        logInfo(s"LOCAL_DISK_HIT\t$blockId\tExecutor $executorId\t$readTime")
      } else {
        logInfo(s"LOCAL_MEM_HIT\t$blockId\tExecutor $executorId\t$readTime")
      }
    }
  }

  def cacheMiss(blockId: BlockId, executorId: String): Unit = {
    logInfo(s"MISS\t$blockId\tExecutor $executorId")
  }
}
